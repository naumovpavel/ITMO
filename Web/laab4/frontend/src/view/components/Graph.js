...
const Graph = () => {
    ...
    const drawPointsByR = (R) => {
        ...
        userInfo.hits.forEach(point => {
                drawnObjects.push(
                    createPoint(point.x, point.y, true, checkInArea(R, point))
                );
        })
    }

    const checkInArea = (R, point) => {
        console.log(R, point);
        if(point.x <= 0 && point.y <= 0) {
            return -2*point.x - point.y <= R && 2*point.x >= -R && point.y >= -R;
        } else if (point.x>= 0 && point.y >= 0) {
            return (point.x*point.x + point.y*point.y) <= R*R;
        } else if (point.x <= 0 && point.y >= 0) {
            return point.x >= -R/2 && point.y <= R;
        }
        return false;
    }
    ...
    return (
        <div className="graphBlock col-md">
            <JXGBoard
                logic={logicJS}
                boardAttributes={{boundingbox: [-5, 3, 3, -3], axis: true}}
                style={{
                    width: "400px",
                    height: "400px"
                }}
