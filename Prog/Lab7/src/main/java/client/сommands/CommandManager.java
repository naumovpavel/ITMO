...
public class CommandManager {
    ...
    public CommandManager(Reader reader, ModelTree tree, ManagerMode mode) {
        ...
        switch (mode) {
            case User -> {
                ...
                this.commands.put("login", new Login(new ModelTree(User.class), new CLIBuilder(reader)));
                this.commands.put("register", new Register(new ModelTree(User.class), new CLIBuilder(reader)));
            }
            case NoUser -> {
                ...
                this.commands.put("login", new Login(new ModelTree(User.class), new NoUserCLIBuilder((BufferedReader) reader)));
                this.commands.put("register", new Register(new ModelTree(User.class), new NoUserCLIBuilder((BufferedReader) reader)));
            }
        }
    }
    ...
    public void startExecuting() {
        ...
        while (reader.hasNextLine()) {
            ...
            if((System.currentTimeMillis() - unixTime) >= times && !client.isConnected()) {
                try {
                    client.connect();
                } catch (IOException e) {
