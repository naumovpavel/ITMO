...
public class AuthManager {
    ...
    public String registerUser(SocketAddress address,  String name, String password) throws RegisterException {
        if(hasInDB(name)) {
            throw new RegisterException("already_registered", address);
        }
        ...
        try (var connection = ConnectionFactory.getConnection()) {
            ...
            throw new RegisterException("register_error", address);
        }
    }

    public boolean hasInDB(String name) {
        try (var connection = ConnectionFactory.getConnection()) {
            PreparedStatement findStatement = connection.prepareStatement(selectUser);
            ...
            Main.logger.warn("ошибка при поиске в базе данных");
        }
        return false;
    }
    ...
    private User findUserInDB(SocketAddress address, String name, String password) throws LoginException {
        ...
        try (var connection = ConnectionFactory.getConnection()) {
            ...
            Main.logger.warn("ошибка при авторизации пользователя с именем {} и адресом {}", name, address);
            throw new LoginException("login_error", address);
        }
    }
