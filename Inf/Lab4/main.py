...
def bench(data, xml_f, xml_lib_f):
    ...
    for j in range(100):
        ...
        task1(data, xml_f)
    print(f"Task 1 time: {time.process_time()    - start}")
    start = time.monotonic()
    for j in range(100):
        del_file_content(xml_lib_f)
        lib_parse(data, xml_lib_f)
    print(f"Task 2 time: {time.monotonic() - start}")
    start = time.monotonic()
    for j in range(100):
        del_file_content(xml_f)
        task3(data, xml_f)
    print(f"Task 3 time: {time.monotonic() - start}")
...
task1(data, open("data.xml", "w+", encoding="utf-8"))
lib_parse(data, xml_lib_f)
